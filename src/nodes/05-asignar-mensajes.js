...
// Todo mensaje generado pasa por un control de calidad: si trae placeholders
// ([Tu Nombre], {{negocio}}, etc.) o viene vacío, se descarta y entra la
...
function textoInvalido(t) {
  const s = String(t || '');
  if (s.trim().length < 80) return true;
...
const plantillas = [
  ...
  (l) => ({
    mensaje: `Hola! Soy ${vendedor}, de Impulso Web (Córdoba). Trabajamos con negocios como ${l.nombre} que todavía no tienen página web. El tema es que sin web dependés del boca a boca, y los clientes nuevos te buscan en Google. Hacemos landings profesionales: $170.000, seña del 50% y en 48hs está online. Lo mejor: primero te muestro una demo de TU web ya armada, gratis. ¿Te interesa verla?`,
...
leads.forEach((l, i) => {
  ...
  if (g && g.mensaje && !textoInvalido(g.mensaje)) {
    ...
    l.followup = !textoInvalido(g.followup) ? String(g.followup) : plantillas[i % plantillas.length](l).followup;
